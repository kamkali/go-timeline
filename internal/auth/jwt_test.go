...
import (
	...
	"github.com/kamkali/go-timeline/internal/logger/mocks"
...
func TestJWTManagerGenerateToken(t *testing.T) {
	...
	t.Run("success creation", func(t *testing.T) {
		j, err := NewJWTManager(mocks.NewLogger(t), testSecretKey, testPubKey)
	...
	t.Run("invalid signing algorithm", func(t *testing.T) {
		j, err := NewJWTManager(mocks.NewLogger(t), testRSASecretKey, testPubKey)
		require.NoError(t, err)
